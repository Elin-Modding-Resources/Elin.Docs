...
1. Mod がローカルの `Package` フォルダにあることを確認します（ワークショップのコピーでは自動エクスポートされません）。
2. ゲームを対象言語に切り替えます——翻訳可能な項目はその場でエクスポートされます（再起動でも同様です）。
...
ゲームは `SourceLocalization.json` に、新しく追加された未翻訳のソースシート項目を自動で追記し、ソースシートに存在しなくなった項目を削除します。
...
通常、ゲームを起動すると `SourceLocalization.json` には未翻訳の新規項目だけが追加されます（ソースシートに存在しなくなった項目は削除されます）。
...
3. ゲームを対象言語に切り替えます。
4. Mod をクリックし、**ローカライゼーション用のテキストをエキスポート** を選びます。
...
> [!WARNING] 注意
> この操作は `SourceLocalization.json` 全体を書き直します。読み込み済みの翻訳は保持されますが、ゲームが最後にファイルを読み込んだ後に手作業で編集した内容は失われます——事前にバックアップしてください。
:::

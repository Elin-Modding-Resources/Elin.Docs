...
**空セルは空の値ではありません**。ゲームは3行目のデフォルト値にフォールバックし、`components` は `log`、`defMat` は `oak`、`category` は `other` が既定値です。
...
|sort|整数|ソート順。例：`2200` で弓の範囲に配置されます。`sort` 列は2つあり、実際に使われるのは後ろのほうです（同名の列は最後のものが優先されます）が、どちらも削除しないでください。|
|_tileType|テキスト|マップ上の表示方法です。下記の [Tile Type](#tile-type) を参照してください。ゲームが知っているタイプ名である必要があり、**未知の値はカード初期化時に例外を投げ、すべてのカード行の読み込みを中断させます**。空欄の場合は `Obj` になります。|
...
|colorType|テキスト|色の参照元：`default`（着色素材のメインカラー）、`alt`（着色素材のオルタカラー）、または `random`。|
|recipeKey|テキスト[]|レシピの入手方法：`*` = デフォルトで習得済み、`-` = ランダムレシピの対象外、ショップタイプ名（`ShopType` 列挙型、例：`Farris`）= そのタイプの商人が販売。|
...
|defMat|テキスト|デフォルト素材（例：`oak`）。アイコン/プレビューの色を決定します。先頭に `!` を付ける（例：`!oak`）と**素材が固定**され、生成時に別の素材へ変わらなくなります。存在しない素材を指定した場合はエラーログを出力して `granite` になります。|
|tierGroup|テキスト|アップグレード/進行のティアグループ。|
|value|整数|基本売却価格（オレン）。|
|LV|整数|アイテムレベル。クラフトに必要なスキルレベルとしても使われます。|
|chance|整数|生成確率の補正値。|
...
|offense|整数[4]|攻撃ステータス：`ダイスの数,ダイスの面,命中補正,ダメージ補正`。例：`2,8,5,4` は 2d8・命中+5・ダメージ+4 です。下記の [攻撃力と防御力](#攻撃力と防御力) を参照してください。|
|substats|整数[]|サブステータス補正。1つ目の値は貫通として使われます。公式の行ではいずれも値を1つだけ指定しています。|
...
|Slope|上り下りできるスロープです。|
|Stairs|Slope と同様ですが、描画と建築ルールが異なります。|
|Paint|壁への取り付けが必要。|
|WallHang|壁への取り付けが必要。|
...
他のプレフィックスの前に重ねられるのは `+` だけです（例：`+#book`、`+$log`）。`$` と `#` を互いに組み合わせることはできません。
...
```
Container,幅,高さ,背景画像,idContainer
```

`idContainer` は省略可能です。アイテム/カテゴリ/spawnlist のIDで、コンテナを初めて開いたときに対応する中身が生成されます。

|例|意味|
|-|-|
|`beekeep,2,2,crate,honey`|2×2のコンテナ（木箱背景）、初回オープン時に蜂蜜を生成。|
|`ChestPractice,7,5,crate`|幅7×高さ5のコンテナ（木箱背景）。|
...
|`Light`|設置型光源。外観・半径は `lightData` プリセットから取得され、パラメータは受け取りません。|
|`LightSource,4`|装備型光源。光の半径。|
...
`offense` は4つの値です：`ダイスの数,ダイスの面,命中補正,ダメージ補正`。`2,8,5,4` は「2d8・命中+5・ダメージ+4」と読みます。`defense` は2つの値：`DV,PV` です。
...
- ダイスの面 ≈ `ダイスの面 × 素材の dice ÷ 基準値`、命中補正 ≈ `命中補正 × 素材の atk × 9 ÷ 基準値`、ダメージ補正 ≈ `ダメージ補正 × 素材の dmg × 5 ÷ 基準値`（`dice`・`atk`・`dmg` は Material シートの列です）。DV ≈ `DV × 素材の dv × 7 ÷ 基準値`、PV ≈ `PV × 素材の pv × 9 ÷ 基準値`。
...
|タグ|効果|
|-|-|
|`exotic`|バニラの異国の品にはこのタグが付いていますが、コードに直接の効果は見つかっていません。|
|`godArtifact`|神器。[信仰テーブル](./religion)を参照。|
|`noWish`|願い（wish）で入手不可。|
|`tourism`|観光アイテムとしてカウントされます。|
|`rareResource`|レア資源として扱われます。|
|`throwWeapon`|専用の投擲武器として扱います（自動攻撃で投擲し、投擲ボーナスが適用されます）。ブーメランのように戻したい場合は、代わりに `elements` 列へ `throwReturn` 要素を追加してください。|
|`noCopy`|複製不可。|
...
+ `IdEffect` は、[マズルフラッシュエフェクトのID](https://gist.github.com/gottyduke/6e2847e37d205a5621bfd0615e5bd9e7#file-elin-effects-md)です。[カスタムエフェクト](../15_Texture%20Mods/effects)を使用できます。デフォルト値は`gunfire`です。ケイン（杖）タイプの武器では使用されませんが、エネルギー（レーザー）ガンでは再生されます。
...
+ `ForceRail` は、ガンに**レールガンアニメーション**の使用を強制します。レーザータイプのガン（特性`ToolRangeGunEnergy`または`ForceLaser`）にのみ効果があります。**これは、特性`ToolRangeGunEnergy`を持つガンのデフォルトの動作ではなくなりました。**

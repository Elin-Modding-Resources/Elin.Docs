...
> [!Note]
> `NoCopy`、`NoRandomSocket`、`Sockets`、`MapStr`、`MapInt` は登録済みの Thing コンテンツ（Thing シートの `tag` 列）用のプロパティであり、ショップ在庫として生成されるアイテムには適用されません。
...
|Currency|通貨。`money`、`money2`、`plat`、`medal`、`casino_coin`、`ecopo`などが指定可能|

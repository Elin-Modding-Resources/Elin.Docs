...
|quality|整数|`0–2`：通常モンスター。`3`：ユニークモンスター（卵入手可能だが友達・捕獲・調教不可）。`4`：ユニークキャラ（名前の表示時に `『』` で囲まれる；受精卵からは雞しか孵化しない；友達にはなれるが、モンスターボールでは捕獲不可）。自作アドベンチャーの場合は記入不要。この値はレア度に対応し、`3` は Mythical、`4` は Artifact です。|
...
|bio|テキスト|スラッシュ区切りの値（空白なし）：`gender`（`m`/`f`/`n`、必須）、`age`、`height`、`weight`、`chara_tone.xlsx`の`tone`、`chara_talk.xlsx`の`talk`。例：`f/51044/152/46/friendly\|私\|あなた`。任意のセグメントは**末尾からのみ**省略できます。詳しくは [bio 列](#bio-列) を参照してください。|
...
本当に必須なのは性別（`m` / `f` / `n`）だけで、任意のセグメントは**末尾からのみ**省略できます。`f////friendly` のような書き方はできません。

...
|job|テキスト|SourceJobの職業ID列から選択。省略時は`none`になります。日本語UIでは「職業」が「クラス」/「職業」として表示される場合があります。 <!-- 表の列名は"job"だが、ゲーム内の日本語表記は「クラス」/「職業」（例：クラス: 魔法戦士）。英語版UIでは"class"。用語の違いを編集者/AIに明示するため残す。 -->|

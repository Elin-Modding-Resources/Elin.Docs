...
|quality|整数|`0–2`：通常モンスター。`3`：ネームドモンスター（名前の表示時に `《》` で囲まれる；受精卵からは同種が孵化する；友達にはなれるが、モンスターボールでは捕獲不可）。`4`：ユニークキャラ（名前の表示時に `『』` で囲まれる；受精卵からは雞しか孵化しない；友達にはなれるが、モンスターボールでは捕獲不可）。この列はコード内のレア度に対応し、`3` は Mythical（神器）、`4` は Artifact（特殊物品）です。自作アドベンチャーの場合は記入不要。|

...
|列名|型|説明|
|-|-|-|
|id|int|エレメントの一意識別子。IDがバニラや他のModのエントリと重複した場合、最後に読み込まれたシートがそれ以前のすべてを上書きします。|
...
|cost|int[]|呪文・アビリティの、レベルスケーリング前の基本コスト。1つ目の値はフィートのフィートポイント消費としても使われ、遺伝子価値やファクション政策のコスト計算にも影響します。|
...
|thing|string|呪文のみ。文字フラグ：`B` = 魔法書、`S` = 巻物、`R` = ロッド、`P` = ポーション、`F` = 香水。呪文がどのアイテム形態で出現するかを指定します。判定は `Contains` なので、バニラの行では文字の位置が固定されています（例：`B SR`）。|
...
|idTrainer|string|未使用——ゲーム内にこの列を読むコードはありません。トレーナーがスキルを教えられるかは、エレメントの `categorySub` とトレーナータイプの一致で決まります。|
|partySkill|int|未使用——ゲーム内にこの列を読むコードはありません。アビリティがパーティ全体を対象とするかは `target` 列（`Party`、`SelfParty`）で決まります。|
...
- **エンチャント：** 出現に必要な危険度を決定します——エンチャントは、その `LV` 以上のレベルの装備にしか付与されません（例：火耐性は `LV` 1 なのでどこでも出現します。なお切断/衝撃耐性は `chance` が 0 のためランダム生成されません）。
...
## textAlt_JP
`string[]`  
値の段階ごとの代替表示名（日本語）。特性/食品エンチャントの表示に使われます：`値/10 + 1` 番目の項目が選ばれ（リスト範囲にクランプ）、`altEnc` 言語文字列でフォーマットされます。

## textAlt
`string[]`  
値の段階ごとの代替表示名（英語）。特性/食品エンチャントの表示に使われます：`値/10 + 1` 番目の項目が選ばれ（リスト範囲にクランプ）、`altEnc` 言語文字列でフォーマットされます。

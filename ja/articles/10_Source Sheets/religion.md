...
エレメントシートを使用して、オプションで神の加護フィートを含めることができます。`featGod_` + `信仰ID` + `1` の形式で名前を付けてください（例：`featGod_custom_spaghettigod1`）。[SourceElement](./element) シートを参照し、`featGod_element1` などの組み込みの加護をコピーして、それを基に行を修正してください。
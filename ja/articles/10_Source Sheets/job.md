...
|ratio|—|職業の強さを概算するマクロ。ゲーム内では未使用です。空白のままにしてください。|
...
|domain|elements|この職業が開始時に持つ領域。エレメントのエイリアスで指定します（例：`eleFire,eleCold,eleLightning`）。領域メカニクスは**プレイヤーキャラクターにのみ適用され**、NPC ではこの列はキャラクター情報ウィンドウの表示にのみ使われます。プレイヤーの場合、**読み込まれるのはエイリアスのみで、後ろに書いた数値は無視されます**。**空欄＝領域なし ではありません**——この列の既定値が `eleFire,eleCold,eleLightning` です。|

...
空のセルは「空の値」ではなく、3行目のデフォルト値にフォールバックします。このシートの既定値は `type`=`Zone`、`LV`=1、`chance`=100、`idBiome`=`Plain`、`dev`=0、`image`=`default` です。
...
|image|テキスト|ゾーン選択画面に表示される画像名で、ゲーム内蔵の画像から選びます。空欄なら `default`。カスタム画像には未対応です。|

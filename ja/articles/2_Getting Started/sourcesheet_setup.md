...
### SourceChara
- Chara - キャラクターのエントリ。
- CharaText - Barks（ポップアップ）。特定の状況でキャラクターに雑談や短いセリフ（バーク）を言わせたい場合があります。Barksはキャラクターの頭上に吹き出しとして表示されます。
...
ソースシートでは、ローカライズテキストを制御する列によって多言語サポートが提供されています。

まずは `name_JP` と `name` のような列グループを例に挙げて、いくつかの基本を理解しましょう：
+ グループ内で `_JP` という接尾辞（サフィックス）が付いている列が日本語の列です。
+ 接尾辞がない列は英語の列ですが、翻訳列として使うこともできます。

### 例

中国語（`CN`）を例として、`CN` フォルダを作成します。
1. `EN` または `JP` フォルダのいずれかでソースシートを記入し、それを `CN` フォルダにコピーします。ソースシートの記入に関する詳細な手順については、メニューの「`Source Sheets`」セクションを参照してください。

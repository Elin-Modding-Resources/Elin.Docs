...
+ `scaleTex` テクスチャのピクセル密度。デフォルトに対する百分率で指定（下記セクション参照）。`0` と `100` はどちらもデフォルトを意味する
...
デフォルトではスプライトは 1 ワールドユニットあたり 100 ピクセルで描画されるため、ピクセル数の多いテクスチャは鮮明になるのではなく単に**大きく**描画されます。`scaleTex` はテクスチャの密度をゲームに伝えるための項目で、これにより高解像度のスプライトをゲーム内で全く同じ大きさのまま使用できます。

ルールは次の式ひとつだけです。ワールドサイズを変えないようにします。

```
ワールドサイズ = キャンバスのピクセル数 / scaleTex * 100
```

| 元の素材 | 高解像度版 | scaleTex | ワールド高さ | 結果 |
| --- | --- | --- | --- | --- |
| 128x256 | 256x512 | 200 | 2.56 | サイズ同一、鮮明さ 2 倍 |
| 128x256 | 512x1024 | 400 | 2.56 | サイズ同一、鮮明さ 4 倍 |
| 128x128 | 384x384 | 300 | 1.28 | サイズ同一、鮮明さ 3 倍 |

`.pref` 内のその他の値（`y`、`heightFix`、`pivotY`、`shadowX`、`equipY` など）はすべてワールドユニットで表されています。ワールドサイズが変わらない限り、**それらはそのまま有効で、調整し直す必要はありません**。これが `scaleTex` の目的です。より鮮明なテクスチャに差し替えても、調整済みの位置設定が無駄になることはありません。

::: warning
`scaleTex` は実際に拡大したテクスチャと対応させる必要があります。元のテクスチャのまま `scaleTex = 200` を設定すると、ワールドサイズが半分になり、スプライトが小さくなるうえに**地面から浮いて見えます**。スプライトは中心を基準点としているため、縮小すると下端が持ち上がるためです。

`scaleTex` を設定して浮いてしまう場合、ほぼ必ずテクスチャが拡大されていないことが原因です。
:::

::: tip
倍率は整数（`200`、`300`、`400`）を推奨します。テクスチャはニアレストネイバー補間で描画されるため、`scaleTex = 80` のような非整数の密度では、あるピクセル行が画面上 1 ピクセル、別の行が 2 ピクセルとなり、輪郭が不均一になります。
:::

なお、タイルとオブジェクトは 1 ワールドユニットあたり 50 ピクセルで描かれており、現状 2 倍に拡大表示されています。これらの素材では `scaleTex = 200` により画面上で真の 1:1 ピクセル対応が得られ、リサンプリングが一切ない最も鮮明な結果になります。

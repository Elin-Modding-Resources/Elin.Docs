...
<!-- 此标题是 character.md 使用的跳转锚点。 -->

制作新角色Mod时，也是通过 `UN_` 开头来锁定角色肖像（固定立绘）。

您应提供命名为 `UN_id.png` 的透明背景图片作为肖像。

+ **尺寸:**  宽:高=3:4   （最好是240x320，否则缩放可能影响图片质量。）
+ **背景：** 透明
+ **文件命名：**  `UN_id.png`   （其中 `id`是您角色的id，即您在 [角色源表](../10_Source%20Sheets/character) 的id列里的填写的内容）

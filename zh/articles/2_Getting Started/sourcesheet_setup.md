...
关于如何填写 xlsx 文件，请移步总目录的 `源表` 分区。<!--Menu=总目录=メニュー-->
...
1. 在EN 或 JP 文件夹之一填写源表，然后再复制到 CN 文件夹（不是官方源表）。源表的详细填写方法请移步总目录的 `源表` 分区。<!--Menu=总目录=メニュー-->
2. 开始翻译你的源表，不要动日文列，把上文无后缀的“翻译列”翻译为中文。（不要动表头，还有其他列作为数据的英语）
...
### 对中国玩家来说
制作mod时还可以：
1. 先在 `CN`文件夹中建立源表。源表的详细填写方法请移步总目录的 `源表` 分区。<!--Menu=总目录=メニュー-->
2. 在翻译列内写中文。关于翻译列，请先阅读[前置知识](#前置知识)

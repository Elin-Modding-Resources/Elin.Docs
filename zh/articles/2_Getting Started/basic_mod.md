...
请填写：您的模组最后可运行的 Elin 主游戏版本。目前若非必要，可不必频繁更新此数值。
...
就这样，一个不执行任何功能的基础空模组已经创建完成。

启动 Elin 并打开 `mod查看`页面，找到你的模组，它应显示为 `[Local]` ，因为它此时是位于 `Package` 文件夹的本地模组。

点击你的模组，然后点击 `公布` 按钮。

如果该模组尚未被发布到创意工坊，它将会被**发布**。若该模组已发布，则会执行**更新**

![publish](./assets/publish.png)

发布模组前请将相关应用保存并关闭。如使用Excel填表时，若不关闭会导致上传更新失败。
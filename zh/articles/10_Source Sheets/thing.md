...
**空格子不等于空值**——游戏会回落到第 3 行的默认值，`components` 默认是 `log`、`defMat` 默认是 `oak`、`category` 默认是 `other`。
...
|sort|整数|排序顺序。例如 `2200` 会将其归入弓的排序范围。有两个 sort 列，实际生效的是靠后的那一个（同名列以最后一个为准），但请注意，你不应删除其中任何一列。|
|_tileType|文本|地图上的显示方式。参见下方 [Tile Type](#tile-type)。必须是游戏认识的类型名——**未知的值会在卡片初始化时抛出异常，中断所有卡片行的加载**。留空则默认为 `Obj`。|
...
|colorType|文本|颜色来源：`default`（取色材质的主色）、`alt`（取色材质的替代色）、`random`（随机）。|
|recipeKey|文本[]|配方获取方式：`*` = 默认已知；`-` = 不进入随机配方池；商店类型名（`ShopType` 枚举，如 `Farris`）= 由该类型商人出售。|
|factory|文本[]|制作该物品的工作台。参见下方 [Factory](#factory)。|
|components|文本[]|合成材料。参见下方 [Components](#components)。|
|disassemble|文本[]|分解后产出的物品。|
|defMat|文本|默认材质（例如 `oak`）。决定图标/预览的颜色。加前缀 `!`（如 `!oak`）表示**锁定材质**，物品不会再随等级或生成方式换材质。填了不存在的材质时游戏会回落到 `granite` 并输出错误日志。|
|tierGroup|文本|升级/进阶的层级分组。|
|value|整数|基础售价（奥伦）。|
|LV|整数|物品等级，同时用作制作所需的技能等级。|
|chance|整数|生成概率修正值。|
...
|offense|整数[4]|攻击属性：`骰子数,骰面,命中加成,伤害加成`。例如 `2,8,5,4` 即 2d8、+5 命中、+4 伤害。参见下方 [攻击与防御数值](#攻击与防御数值)。|
|substats|整数[]|副属性修正，第一个值用作穿透。本体源表里都只填了一个数值。|
...
- 文件名必须与 `id` 完全一致。
- 使用小写 `.png` 扩展名（`.PNG` 无效）。
- 放置在 Mod 的 `Texture` 文件夹**根目录**下。**子目录不会被找到**，除非 `id` 本身就带那个前缀。
...
只有 `+` 前缀可以叠加在其他前缀之前，例如 `+#book` 或 `+$log`；`$` 与 `#` 不能互相组合。
...
```
Container,宽,高,背景图片,idContainer
```

`idContainer` 为可选项：物品/类别/spawnlist 的 ID，首次打开容器时会把对应内容预生成在容器内。

|示例|含义|
|-|-|
|`beekeep,2,2,crate,honey`|2×2 容器（木箱背景），首次打开时预生成蜂蜜。|
|`ChestPractice,7,5,crate`|宽 7 × 高 5 容器（木箱背景）。|
...
|`Light`|放置型光源，外观/半径来自 `lightData` 预设，不接受参数。|
|`LightSource,4`|装备型光源，发光半径。|
...
`offense` 是四个值：`骰子数,骰面,命中加成,伤害加成`。`2,8,5,4` 读作「2d8，+5 命中，+4 伤害」。`defense` 是两个值：`DV,PV`。
...
- 骰面 ≈ `骰面 × 材质的 dice ÷ 基准值`，命中加成 ≈ `命中加成 × 材质的 atk × 9 ÷ 基准值`，伤害加成 ≈ `伤害加成 × 材质的 dmg × 5 ÷ 基准值`（`dice`、`atk`、`dmg` 都是 Material 表里的列）；DV ≈ `DV × 材质的 dv × 7 ÷ 基准值`，PV ≈ `PV × 材质的 pv × 9 ÷ 基准值`。
...
|标签|效果|
|-|-|
|`exotic`|本体的异国货物品带有此 tag，代码中未发现直接效果。|
|`godArtifact`|神器。参考[自定义信仰](./religion)。|
|`noWish`|无法通过许愿获得。|
|`tourism`|计入观光物品。|
|`rareResource`|视为稀有资源。|
|`throwWeapon`|标记为专职投掷武器（自动攻击时投掷、享受投掷加成）。想让武器像回旋镖一样飞回，请在 `elements` 列添加 `throwReturn` 元素。|
|`noCopy`|不能复制。|
|`noShop`|在对应类型的商店中不生成。|
...
+ `Num` 是一个爆发中的射击次数。
+ `Delay` 是动画延迟，单位为秒。
+ `IdEffect` 是**枪口特效的 ID**。您可以使用[自定义特效](../15_Texture%20Mods/effects)。默认值是 `gunfire`。法杖不使用此值；能量（激光）枪仍会播放。
...
+ `ForceRail` **强制**枪械使用**电磁炮动画**。仅对激光类枪械（特性 `ToolRangeGunEnergy` 或 `ForceLaser`）生效。**对于带有 `ToolRangeGunEnergy` 特性的枪械来说，这不再是默认行为。**

...
|domain|elements|此职业初始拥有的领域，填元素别名（例如 `eleFire,eleCold,eleLightning`）。领域机制**只对玩家角色生效**，NPC 的这一列仅用于其角色信息窗口的显示；对玩家而言**只读取元素别名，写在后面的数值会被忽略**。**留空不代表没有领域**——该列默认值就是 `eleFire,eleCold,eleLightning`。|

...
|defMat|文本|默认尸体材质，来自 SourceBlock/Material 内 alias 列。留空则使用种族的默认材质。|
|LV|整数|角色的“危险等级”；影响根据地图危险度生成的生成阈值、选择成本（奴隶主/驯兽师）以及基于种族/职业特征的基础属性生成。|
|chance|整数|地图生成几率的修正值（可能也影响销售列表）。默认值为 `100`。|
|quality|整数|`0–2`：普通等级。`3`：独特怪物（可获得蛋；无法成为朋友/捕捉/驯服）。`4`：独特角色（名字中含有`『』`；受精卵仅能孵化出鸡；可以成为朋友但无法使用精灵球捕捉）。自定义冒险者无需填写。|
...
|race|文本|来自 SourceChara/Race 的种族 ID。|
|job|文本|来自 SourceChara/Job 的职业 ID；默认为 `none`。|
...
|actCombat|文本|战斗中可使用的主动能力/魔法，来自 SourceGame/Element 条目，用半角逗号分隔。添加 `/N` 可设置固定使用概率。增益效果可添加 `/pt` 使其作用于整个队伍（仅限友方状态）。示例：`ActThrowPotion/30,SpWeakness,SpSpeedDown,SpWisdom/50/pt`。默认概率为 100。|
|mainElement|文本|主要元素亲和力：`Fire`、`Cold`、`Lightning`、`Darkness`、`Nether`、`Sound`、`Chaos`、`Poison`、`Cut`、`Acid`、`Impact`。|
|elements|文本|被动如专长/附魔，来自 SourceGame/Element 条目，用半角逗号分隔。使用时添加 `/N` 表示等级/数值。`0` 或负值可修改继承自种族的元素。示例：`invisibility/1` 为启用，`invisibility/0` 为禁用继承效果；`antidote/-30` 会让肉带毒，`antidote/30` 可解毒或抵消种族的 `-30`。|
...
|works|文本|来自 SourceChara/Hobby 的 alias。|
|hobbies|文本|来自 SourceChara/Hobby 的 alias。|
...
肖像也叫立绘；是与角色对话时，弹窗中左侧显示图片。

肖像应放置于 `Portrait` 文件夹中，而 `Portrait`文件夹应位于的你[模组包](../2_Getting%20Started/basic_mod)中

关于肖像的详细内容，请移步 [肖像](../15_Texture%20Mods/portraits#新角色Mod的肖像)，阅读角色mod肖像章节。

### 贴图（Sprite）

角色在地图上的贴图，更准确的描述是精灵图Sprite

当你准备给你的mod角色提供一张新的图片作为角色贴图（Sprite）时，首先，你应在源表的 `_idRenderData` 列填入 `@chara` 。

角色贴图（Sprite）为一张透明背景的 `.png` 图片，应放入 `Texture` 文件夹。而 `Texture` 文件夹应位于的你 `游戏安装目录/Elin/Package/自定义的mod文件夹名字` 中（其中 `自定义的mod文件夹名字` 即你的 [模组包](../2_Getting%20Started/basic_mod)。

一般来说，角色贴图（Sprite）的命名应为 `ID.png`，ID正是你的角色ID。

您还可以使用动画贴图，更大尺寸的画布，以及不同条件下变化的贴图变体；详细内容，请移步总目录的 `贴图模组` 分区阅读。<!--Menu=总目录=メニュー。Texture Mods=贴图模组=テクスチャMOD--> 

### 示例

关于肖像和贴图，你可以参考Tiny Mita范例：

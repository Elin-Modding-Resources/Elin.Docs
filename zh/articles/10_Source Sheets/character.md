...
角色贴图（Sprite）为一张透明背景、任意尺寸的 `.png` 图片，应放入 `Texture` 文件夹。而 `Texture` 文件夹应位于的你 `游戏安装目录/Elin/Package/自定义的mod文件夹名字` 中（其中 `自定义的mod文件夹名字` 即你的 [模组包](../2_Getting%20Started/basic_mod)。

您还可以使用动画贴图，更大尺寸的画布，以及不同条件下变化的贴图变体。

详细内容，请移步总目录的 `贴图模组` 分区阅读。<!--Menu=总目录=メニュー。Texture Mods=贴图模组=テクスチャMOD--> 

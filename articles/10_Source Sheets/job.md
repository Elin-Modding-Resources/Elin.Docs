...
|ratio|—|Macro to approximate the job strength; unused in-game. Leave blank.|
...
|domain|elements|Which domains this job starts with, given as element aliases (e.g. `eleFire,eleCold,eleLightning`). The domain mechanic **only applies to the player character**; for NPCs the column is display-only (shown in their character window) — and **only the element aliases are read; any values written after them are ignored**. **Leaving it blank does not mean no domains** — the column defaults to `eleFire,eleCold,eleLightning`.|

...
音频文件应为 **acc**、**mp3**、**ogg**、**wav** 格式之一，文件名作为音频ID。加载时会生成默认的同名元数据JSON，允许您编辑并在在下次游戏启动时应用音频文件元数据。
...
**重要说明，** 这个 `id` 仅用于BGM。您的音效 ID 仍然是不含拓展名的文件名。
...
CWL 提供了一组游戏内的控制台命令，以便您更轻松地编辑播放列表。加载存档后，打开控制台并使用 `cwl.bgm.view` 查看当前区域的播放列表和曲目。

![](../../assets/cwl_bgm_view.png)

您还可以使用 `cwl.bgm.next`、`cwl.bgm.last`、`cwl.bgm.shuffle` 来测试您的播放列表。当您对自己的播放列表效果感到满意时，请使用 `cwl.bgm.hide` 关闭弹出窗口。

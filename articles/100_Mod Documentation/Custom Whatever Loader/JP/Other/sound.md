...
オーディオファイルは **acc**、**mp3**、**ogg**、**wav** 形式で、ファイル名がオーディオIDとして使用されます。読み込まれると、デフォルトの同名メタデータJSONが生成され、編集して次回ゲーム起動時にオーディオファイルのメタデータを適用できます。
...
プレイリストのJSONファイル名は以下のいずれかと一致する必要があります：

+ 既存のプレイリスト名（`Blank` を除く）
+ 地域タイプ名
+ `"Global"`
...
例えば、すべてのダンジョンZone（`Zone_RandomDungeon`、`Zone_RandomDungeonFactory`、`Zone_Mine` など）は、`Dungeon` という名前のプレイリストを共有しています。このプレイリストに曲を追加または削除したい場合は、**Sound/BGM/Playlist/** フォルダーに `Dungeon.json` を用意する必要があります。

![](../../assets/playlists.png)

`Dungeon.json` での変更は、ゲームの `Dungeon` プレイリストにマージされ、曲の追加または削除が行われます。これは、このプレイリストを共有するすべてのZoneに影響します。

もう一つの一般的な使用例は、あなたが大マップにいるときに曲を追加することです。大マップはZoneタイプ `Region` で、プレイリスト `EloMap` を持っています。

### Zoneのオーバーライド

プレイリストのマージに加えて、各Zoneタイプに対してZoneのオーバーライドを指定することもできます。これらのプレイリストはZoneタイプ名を使用し、その内容はZoneのデフォルトプレイリストにマージされます（表に指定がない場合は `Blank` です）。

したがって、`EloMap.json` ではなく `Region.json` を使用して、大マップのプレイリストに曲を追加することもできます。

### グローバルオーバーライド

すべてのプレイリストにマージされる特別なプレイリスト `Global` を用意することもできます。

3つのプレイリストは同時に存在でき、グローバルオーバーライド、プレイリストのマージ、Zoneのオーバーライドの順序で効果を発揮します。重複する曲は削除されます。プレイリストの効果の順序は、Modのロード順序にも影響されます。
...
```json
{
    "shuffle": true,
    "list": [
        "My new BGM Sound ID 1",
        "My new BGM Sound ID 2"
    ],
    "remove": [
        "006 elomap2",
        "007 elomap3",

...
CWL **1.21.0** 以降、ドラマシートは [スクリプト機能](../Other/scripting) をサポートします。これにより、ドラマシート内で `eval` アクションを使って **C# スクリプト** を直接記述できるようになりました。

この機能は通常の CWL スクリプトと同じ機能を提供しますが、重要な違いがあります：
**スクリプトの状態は現在のドラマインスタンスに紐づけられ**、ドラマの進行中は保持され、ドラマ終了時に **自動的にリセット** されます。

![](./assets/drama_eval.png)

スクリプトが `bool` 型の値を返す場合、その値は指定された `jump` ターゲットを実行するかどうかの判断に使用されます。

同じドラマシートのフォルダ内にあるスクリプトファイルを読み込む場合は、`eval <<<script.cs` を使用することもできます。

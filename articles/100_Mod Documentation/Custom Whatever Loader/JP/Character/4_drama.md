...
|`show_book`|書籍ID, カテゴリ(`Book` または `Scroll`)|本を開く、**LangMod/_*_*/Text**フォルダーをサポート、例えば `Text/Book/ok.txt` の場合は `(ok, Book)` を使用|成功時|
...
|`mod_element`|[元素alias](https://docs.google.com/spreadsheets/d/16-LkHtVqjuN9U0rripjBn-nYwyqqSGg_/edit?gid=1766305727#gid=1766305727), 強度（オプション）|`actor`の指定された要素（特性/抵抗/スキルなど）を変更し、デフォルトの強度は`1`であり、異なるタイプの要素には異なる強度が使用される|常時|

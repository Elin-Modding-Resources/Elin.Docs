...
|メソッド|パラメータ|説明|ジャンプ条件|
|-|-|-|-|
|`choice`|拡張条件|選択行を条件付きで有効化する、例えば `choice(if_lv(>=10))`。**推奨**されるのは、`choice` アクション（`invoke*` の代わりに）を使用し、param を拡張されたアクションに設定すること|条件を満たす場合|
|`eval`|C# スクリプト|C# スクリプトまたはファイルを `<<<path.cs` 構文で実行する。**推奨**されるのは、`eval` アクション（`invoke*` の代わりに）を使用し、param を C# スクリプトに設定すること|`true` を返す場合|

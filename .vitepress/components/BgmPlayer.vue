<template>
  <div class="bgm-player flex flex-col h-160 bg-zinc-950 dark:bg-neutral-900 rounded-3xl overflow-hidden border border-zinc-800 dark:border-neutral-700 shadow-2xl">
    
    <div class="flex flex-1 min-h-0">
      <div class="w-65 bg-zinc-900 dark:bg-neutral-950 flex flex-col border-r border-zinc-800 dark:border-neutral-700">
        <div class="px-6 pt-6">
          <div class="text-xs font-semibold tracking-widest text-zinc-500 dark:text-neutral-400 mb-3">
            {{ t("library") }}
          </div>
          ...
          <div
            ...
            :class="currentView === 'all' ? 'bg-violet-500 text-white shadow-md shadow-violet-500/30' : 'hover:bg-zinc-800 dark:hover:bg-neutral-800 text-zinc-300 dark:text-neutral-300'"
          >
            <span class="text-xl">♪</span>
            <span class="font-medium">{{ t("allTracks") }} ({{ bgmItems.length }})</span>
          </div>
        </div>
        ...
        <div class="flex-1 flex flex-col min-h-0 px-6 pt-5">
          ...
          <div class="flex-1 overflow-y-auto custom-scroll space-y-1 pr-2">
            <div
              ...
              :class="currentPlaylist?.name === playlist.name ? 'bg-violet-500 text-white shadow-md shadow-violet-500/30' : 'hover:bg-zinc-800 dark:hover:bg-neutral-800 text-zinc-300 dark:text-neutral-300'"
            >
              <span class="text-xl">📁</span>
      ...
      <div class="flex-1 flex flex-col bg-zinc-950 dark:bg-neutral-900">
        <div class="h-14 border-b border-zinc-800 dark:border-neutral-700 bg-zinc-900/70 dark:bg-neutral-900/70 flex items-center px-4 gap-3">
          <div class="font-semibold text-zinc-100 dark:text-neutral-100 truncate min-w-0 flex-1">
            {{ currentTitle }}
          </div>
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="text-xs px-3 py-1 rounded-full bg-zinc-800 dark:bg-neutral-800 text-zinc-400 whitespace-nowrap">
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-1 custom-scroll">
          <div
            ...
            :class="{ 'bg-violet-500/10 dark:bg-violet-500/15': isCurrentTrack(track) }"
          >
            <div class="w-8 text-right font-mono text-sm text-zinc-500 dark:text-neutral-500 group-hover:text-violet-400 transition-colors"
                 :class="{ 'text-violet-400': isCurrentTrack(track) }">
              {{ (getOriginalIndex(track) + 1).toString().padStart(2, "0") }}
            </div>

            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-zinc-200 dark:text-neutral-200 truncate"
                   :class="{ 'text-violet-300 dark:text-violet-400': isCurrentTrack(track) }">
                {{ track.name }}
              </div>
              <div class="text-xs text-zinc-500 dark:text-neutral-500 truncate">
                #{{ track.id }} {{ track.author?.replace(/&\s*$/, "") || "Unknown" }}
              </div>
            </div>

            <div v-if="isCurrentTrack(track)" class="flex items-center gap-2 text-violet-400">
              <div class="w-2 h-2 rounded-full bg-violet-400 animate-pulse"></div>
              <span class="text-xs font-medium">PLAYING</span>
            </div>
          </div>

          <div v-if="filteredTracks.length === 0" class="text-center py-12 text-zinc-500">
            NONE
          </div>
        </div>
      </div>
    ...
    <div class="border-t border-zinc-800 dark:border-neutral-700 bg-zinc-900 dark:bg-neutral-950">
...
const playlistModules = import.meta.glob("/playlist/Playlist_*.json", { eager: true, import: "default" });

const playlists = Object.values(playlistModules) as { name: string; entries: number[] }[];
...
const { lang } = useData();
const t = (key: keyof typeof i18n.zh) => i18n[lang.value as keyof typeof i18n][key];
...
<style>
.aplayer-title { color: blueviolet !important; }
</style>